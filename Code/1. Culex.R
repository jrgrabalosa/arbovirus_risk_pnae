...
# loo1 <- loo(culex_model_zinb)
# loo2 <- loo(culex_model_zinb_2)
# 
...
cat_st <- st_read(paste0("Data/Land_covers/area_interes_shp.shp")) %>%
  st_transform(32631) 
...
# Read prediction
pred <- read.csv("Prediction/Culex/pred.csv")
...
temporal_variability_plot <- ggplot(data = agg_newdata, aes(x = week, y = pp, color = year, group = year)) +
  ...
  scale_y_continuous(breaks = c(15, 30, 45, 60)) + # Set breaks for the Y-axis
  ...
  theme(
    ...
    panel.grid.minor.y = element_blank(), # Light gray for minor horizontal grid lines
...
filename <- "Plots/Culex/plot_2015.jpg"
...
filename <- "Plots/Culex/plot_2016.jpg"
ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
...
filename <- "Plots/Culex/plot_2017.jpg"
ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
...
filename <- "Plots/Culex/plot_2018.jpg"
ggsave(filename, dpi = 300, width = 15, height = 6, units = "in", type = "jpg", quality = 100)
